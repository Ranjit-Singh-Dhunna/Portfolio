...
function DitherButton({
  ...
  useEffect(() => {
    ...
    const draw = (timeSec: number) => {
      ...
      const angle = Math.sin(timeSec * 0.15) * 0.112;
      ...
      for (let py = 0; py < h; py++) {
        for (let px = 0; px < w; px++) {
          ...
          const posY = (py - cy) * ps;
          const value =
            Math.sin((posX * dx + posY * dy) * 0.048 - timeSec * 0.35) * 0.5 +
            0.5;
          ...
          if (on) {
            ...
            // Draw transparent for non-dithered areas so button background shows through
            data[i] = 0;
            data[i + 1] = 0;
            data[i + 2] = 0;
            data[i + 3] = 0;
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        aria-hidden
        style={{ opacity: ditherOpacity, position: 'absolute', inset: 0, width: '100%', height: '100%' }}
        className="pointer-events-none text-foreground [image-rendering:pixelated]"
      />
